...
const DropdownUser = () => {
  ...
  const cookiesToRemove = ["isLoggedIn", "token", "attributes"];

  const handleLogout = () => {
    cookiesToRemove.forEach((cookieName) => {
      Cookies.remove(cookieName, {
        path: "/", // Specify the path where the cookie was set
  ...
  useEffect(() => {
    const temp = Cookies.get("attributes");
    const cookiesData = JSON.parse(temp as string);
    setStoreInfo(cookiesData);
  }, []);
    ...
    <ClickOutside onClick={() => setDropdownOpen(false)} className="relative">
      ...
      >
        ...
        <span className="flex items-center gap-2 font-medium text-dark dark:text-dark-6">
          <span className="hidden lg:block">{storeInfo?.storeName}</span>
